...
void DrawNode(ImNodesEditorContext& editor, const int node_idx)
{
    ...
    {
        // node base
      GImNodes->CanvasDrawList->AddRectFilled(node.Rect.Min, node.Rect.Max, node_background, node.LayoutStyle.CornerRounding);
      //GImNodes->CanvasDrawList->AddCircleFilled(node.Rect.GetCenter(), 100,  node_background, 6);
...
#else
            GImNodes->CanvasDrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, titlebar_background, node.LayoutStyle.CornerRounding,ImDrawFlags_RoundCornersTop);
//            GImNodes->CanvasDrawList->AddCircleFilled(title_bar_rect.GetCenter(), 100, titlebar_background, 6);
#endif
        }
